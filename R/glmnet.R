...
fit_glmnet = function(Y_list,
  ...
  for (a in 1:n_alpha) {
    ...
    fit = glmnet::glmnet(x = X, y = Y, family = "multinomial", type.multinomial = "grouped", nlambda = n_lambda, lambda.min.ratio = lambda_min_ratio, alpha = alpha_seq[a], standardize = FALSE, maxit = 1e7, thresh = 1e-8)
