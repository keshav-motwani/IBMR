...
system.time({fit = glmnet(do.call(rbind, X_list), unlist(Y_list), family = "multinomial", alpha = 1, standardize = TRUE, intercept = TRUE, type.multinomial = "grouped", nlambda = 20, lambda.min.ratio = 1e-3, maxit = 1e6, thresh = 1e-8)})
...
test2 = fit_glmnet(Y_list, category_mappings$categories, X_list, n_lambda = 20, lambda_min_ratio = 1e-3)

plot(coef(fit, fit$lambda[10])[[1]][-1], test2$model_fits[[1]][[10]]$Beta[, 1])
abline(0, 1)
